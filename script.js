...
class homeContent {
  constructor() {
    ...
    this.todoList = [
      new todoItem(
        ...
        this.editTodoItem.bind(this)
      ),
      new todoItem(
        "BoomMom",
        "wassssussppp bitches",
        "Apr 29th",
        "medium",
        this.deleteItem.bind(this),
        this.editTodoItem.bind(this)
  ...
  loadTodoItems() {
    ...
    if (savedItems !== null) {
      this.todoList = JSON.parse(savedItems).map(
        (item) =>
          new todoItem(
            item.title,
            item.desc,
  ...
  addTodoItem() {
    ...
    submitForm.addEventListener("submit", (e) => {
      ...
      if (this.todoList.length == 8) {
        ...
        const checkIfExists = this.todoList.some(
          (object) => object.title.toLowerCase() === newTitle.toLowerCase()
        );
        if (checkIfExists == false) {
          this.todoList.push(
            new todoItem(
              ...
              formatDate(e.target.date.value),
  ...
  defaultText() {
    if (this.todoList.length == 0) {
      this.homeHeader.innerHTML =
        "This is currently empty. Click the lower left hand plus symbol to add a new task!";
    } else {
      this.homeHeader.innerHTML = "";
    }
  }
...
class projectsContent {
  ...
  loadProjects() {
    ...
    if (savedProjects !== null) {
      this.projectObjects = JSON.parse(savedProjects).map(
        (name) => new projectContent(name)
      );
    } else {
      this.projectObjects = [];
    }
  ...
  saveProjects() {
    localStorage.setItem(
      "projectObjects",
      JSON.stringify(this.projectObjects.map((proj) => proj.name))
    );
  }
  ...
  createElements() {
    ...
    this.popup.className = "popup";
    this.popup.innerHTML =
      "<h2>New Project</h2> <form class='input-fields-projects'> <label for='title'>Title:</label> <input type='text' placeholder='2-10 characters max' id='title' name='title' minlength='2' maxlength='10' required> <button type='submit' id='add-to-do' class='add-to-do'>Add Project</button> </form>";
    this.backdrop = document.createElement("div");
    this.backdrop.className = "backdrop";
    this.addButton = document.querySelector(".add-button");
  }
  ...
  addProjectEventListeners(name) {
    ...
    this.deleteProjectButton.addEventListener("click", () => {
      this.projectObjects = this.projectObjects.filter(
        (item) => item.name !== name
      );
      this.saveProjects();
    ...
    if (this.submitListenerAdded !== true && this.active == true) {
      submitProject.addEventListener("submit", (e) => {
        ...
        if (this.projectObjects.length == 10) {
          ...
          const newTitle = formatTitle(e.target.title.value);
          const checkIfExists = this.projectObjects.some(
            (object) => object.name.toLowerCase() === newTitle.toLowerCase()
          );
          if (checkIfExists == false) {
...
class projectContent {
  ...
  loadTodoItems() {
    ...
    if (savedProjectItems !== null) {
      this.todoList = JSON.parse(savedProjectItems).map(
        (item) =>
          new todoItem(
            item.title,
            item.desc,
  ...
  defaultText() {
    if (this.todoList.length == 0) {
      this.projectHeader.innerHTML =
        "This project is currently empty. Click the lower left hand plus symbol to add a new task!";
    } else {
      this.projectHeader.innerHTML = "";
    }
  }
